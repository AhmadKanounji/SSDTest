...
SSD_ISSUE_TYPE_ID = "12627"
SSD_VERSION_FIELD = "customfield_13057"
...
def render_confluence_image_from_attachment(att):
    ...
    return (
        f'<p>'
        f'<ac:image ac:width="1000" ac:layout="center">'
...
def create_ssd_issue(release_version, release_author, trigger_issue_key):
    url = f"{JIRA_BASE}/rest/api/3/issue"

    summary = f"[SSD] - Version {release_version}"

    description = (
        f"Automatically generated SSD release.\n\n"
        f"Version: {release_version}\n"
        f"Generated By: {release_author}\n"
        f"Generated From Trigger: {trigger_issue_key}"
    )
    ...
    log(f"create_ssd_issue payload = {payload}")
    ...
    log(f"create_ssd_issue response: {r.status_code}")
    log(f"create_ssd_issue body: {r.text[:1000]}")
...
def generate_docx():
    try:
        ...
        ssd_issue_key = create_ssd_issue(
            release_version=release_version,
            release_author=author,
            trigger_issue_key=pending_issue_key
        )
        ...
        log(f"Attached DOCX to SSD issue {ssd_issue_key}")
